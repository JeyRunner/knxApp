...
void DisconnectRequest::toBytes(char bytes[])
{
    ...
    bytes[10] = ntohl(connection->clientAddr.sin_addr.s_addr) >> 24 & 0xff;     // c0 - IP address c0 = 192 */
    bytes[11] = ntohl(connection->clientAddr.sin_addr.s_addr) >> 16 & 0xff;     // a8 - IP address a8 = 168 */
    bytes[12] = ntohl(connection->clientAddr.sin_addr.s_addr) >> 8 & 0xff;      // 0a - IP address 0a = 10 */
    bytes[13] = ntohl(connection->clientAddr.sin_addr.s_addr) & 0xff;           // b3 - IP address 9F = 179 */
    bytes[14] = (ntohs(connection->clientAddr.sin_port) >> 8) & 0xff;           // 0e - hi-byte local port number for CONNECTION, CONNECTIONSTAT and DISCONNECT requests */
    bytes[15] = ntohs(connection->clientAddr.sin_port) & 0xff;                  // 57 - lo-byte local port number for CONNECTION, CONNECTIONSTAT and DISCONNECT requests */
}
